...
con = sqlite3.connect('data/tetris_records.db')
cur = con.cursor()
...
def start_screen():
    global title_sound_play

    if title_sound_play is False:
    ...
    start_game = load_image('button_start_game.jpg', -1)
    start_game = pygame.transform.scale(start_game, (200, 70))
    screen.blit(start_game, (60, 300))
    table_of_records = load_image('button_table_record.jpg', -1)
    table_of_records = pygame.transform.scale(table_of_records, (200, 70))
    screen.blit(table_of_records, (60, 370))
    exit_game = load_image('button_exit_game.jpg', -1)
    exit_game = pygame.transform.scale(exit_game, (200, 70))
    screen.blit(exit_game, (60, 440))
...
def level_hard():
    ...
    difficult1 = load_image('button_1.jpg', -1)
    difficult1 = pygame.transform.scale(difficult1, (200, 70))
    screen.blit(difficult1, (60, 300))
    difficult2 = load_image('button_2.jpg', -1)
    difficult2 = pygame.transform.scale(difficult2, (200, 70))
    screen.blit(difficult2, (60, 370))
    difficult3 = load_image('button_3.jpg', -1)
    difficult3 = pygame.transform.scale(difficult3, (200, 70))
    screen.blit(difficult3, (60, 440))
    back = load_image('button_back.jpg', -1)
    back = pygame.transform.scale(back, (200, 70))
    screen.blit(back, (60, 510))
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if 60 <= event.pos[0] <= 260 and 440 <= event.pos[1] <= 510:
                    difficult = 4
                    return
                if 60 <= event.pos[0] <= 260 and 510 <= event.pos[1] <= 580:
                    start_screen()
                    return
        pygame.display.flip()
        clock.tick(FPS)
...
def show_table_of_records():
    ...
    back = load_image('button_back.jpg', -1)
    back = pygame.transform.scale(back, (200, 70))
    screen.blit(back, (60, 620))
    table = load_image('table_of_record.png', -1)
    table = pygame.transform.scale(table, (300, 200))
    screen.blit(table, (10, 10))
    screen.blit(table, (10, 210))
    screen.blit(table, (10, 410))

    con = sqlite3.connect('data/tetris_records.db')
    cur = con.cursor()
    records_1 = cur.execute("SELECT * FROM records_1").fetchall()
    records_2 = cur.execute("SELECT * FROM records_2").fetchall()
    records_3 = cur.execute("SELECT * FROM records_3").fetchall()
    records_all = [records_1, records_2, records_3]
    coord_x = 48
    coord_y = 25
    for number, difficult in enumerate(records_all):
        string_rendered = font.render("Сложность " + str(number + 1), True, pygame.Color('black'))
        screen.blit(string_rendered, [coord_x, coord_y])
        coord_y += 40
        for record in difficult:
            for el in record:
                string_rendered = font.render(str(el), True, pygame.Color('black'))
                screen.blit(string_rendered, [coord_x, coord_y])
                coord_x += 60
            coord_y += 50
            coord_x -= 120
    ...
    while True:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if 60 <= event.pos[0] <= 260 and 620 <= event.pos[1] <= 690:
                    start_screen()
                    return
        pygame.display.flip()
        clock.tick(FPS)
        ...
        if game.running_game is False:
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_ESCAPE:
                    ...
                    last_dif = game.difficulty
                    if last_dif == 2:
                        rec_dif_1 = cur.execute("SELECT * FROM records_1").fetchall()
                        for num, el in enumerate(rec_dif_1):
                            if last_score >= el[1]:
                                pre_score = el[1]
                                cur.execute(f'UPDATE records_1 SET score=? WHERE place=?', (last_score, num + 1))
                                num += 1
                                while num < 2:
                                    cur.execute(f'UPDATE records_1 SET score=? WHERE place=?', (pre_score, num + 1))
                                    num += 1
                                break
                    if last_dif == 3:
                        rec_dif_2 = cur.execute("SELECT * FROM records_2").fetchall()
                        for num, el in enumerate(rec_dif_2):
                            if last_score >= el[1]:
                                pre_score = el[1]
                                cur.execute(f'UPDATE records_2 SET score=? WHERE place=?', (last_score, num + 1))
                                num += 1
                                while num < 2:
                                    cur.execute(f'UPDATE records_2 SET score=? WHERE place=?', (pre_score, num + 1))
                                    num += 1
                                break
                    if last_dif == 4:
                        rec_dif_3 = cur.execute("SELECT * FROM records_3").fetchall()
                        for num, el in enumerate(rec_dif_3):
                            if last_score >= el[1]:
                                pre_score = el[1]
                                cur.execute(f'UPDATE records_3 SET score=? WHERE place=?', (last_score, num + 1))
                                num += 1
                                while num < 2:
                                    cur.execute(f'UPDATE records_3 SET score=? WHERE place=?', (pre_score, num + 1))
                                    num += 1
                                break
                    con.commit()
